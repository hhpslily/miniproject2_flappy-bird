    ...
    : QMainWindow(parent)
{
    ...
    //音樂設定
    bgm = new easyMusic("music/mission_short.wav",80,1);
    fgm = new easyMusic("music/sfx_wing.mp3",100,0);
}
void MainWindow::paintEvent(QPaintEvent *)		//繪圖事件, 用来產生背景
{
    ...
    if(gamemod==3){
        QPixmap in;
        in.load(":/Image/in.jpg");
        painter.drawPixmap(0,0,380,450,in);
    }
    else if(gamemod==lose){
        QPixmap over;
        over.load(":/Image/over.jpg");
        painter.drawPixmap(0,0,380,450,over);
    }
    else{
        if(flag==0){
            QPixmap bg;
            ...
            QPixmap bg2;
            bg2.load(":/Image/bg2.jpg");
            painter.drawPixmap(0,0,380,450,bg2);
        }
    }
}
...
{
    // 在這裡設定按下滑鼠要觸發的功能
    if(event->button()==Qt::LeftButton){ //左鍵暫停
        birdTimer->stop();
        pipeTimer->stop();
    }
    if(event->button()==Qt::RightButton){ //右鍵恢復
        birdTimer->start();
        pipeTimer->start();
    }
...
void MainWindow::birdAction()
{
    ...
    //上界偵測
    if(birds->pos().y()+birds->height()<=birds->height())
    {
        birds->move(birds->pos().x(),win_height-birds->height()+10); // XX->height可用來取得物件高度
        birdTimer->stop();
        gameLose();
